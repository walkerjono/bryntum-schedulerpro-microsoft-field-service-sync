...
function makeDatum({ effort = 0, maxEffort = 100, isGroup = false, resource = undefined as TreeGroupResource | undefined, startDate = new Date('2026-03-01') } = {}): AllocationDatum {
...
describe('getLeafDescendants', () => {
    it('returns [self] for a leaf resource', () => {
        const leaf: TreeGroupResource = { id : '1', isLeaf : true };
        expect(getLeafDescendants(leaf)).toEqual([leaf]);
    });

    it('returns [self] when resource has no children property', () => {
        const leaf: TreeGroupResource = { id : '2' };
        expect(getLeafDescendants(leaf)).toEqual([leaf]);
    });

    it('returns empty array for non-leaf resource with no children', () => {
        const parent: TreeGroupResource = { id : '3', isLeaf : false, children : [] };
        expect(getLeafDescendants(parent)).toEqual([]);
    });

    it('returns all leaf descendants for a nested tree', () => {
        const a: TreeGroupResource = { id : '10', isLeaf : true };
        const b: TreeGroupResource = { id : '11', isLeaf : true };
        const c: TreeGroupResource = { id : '12', isLeaf : true };
        const mid: TreeGroupResource = { id : '20', children : [b, c] };
        const root: TreeGroupResource = { id : '30', children : [a, mid] };
    ...
    it('skips non-leaf children without children array', () => {
        const nonLeaf: TreeGroupResource = { id : '5', isLeaf : false, children : [] };
        const child: TreeGroupResource = { id : '6', isLeaf : true };
        const root: TreeGroupResource = { id : '7', children : [nonLeaf, child] };
...
describe('clearLeafStateCache', () => {
    ...
    it('resets parent classification to mixed-state on next render', () => {
        ...
        const resource: TreeGroupResource = { id : 'R1' };
        ...
        const parentResource: TreeGroupResource = { id : 'P1', children : [resource] };
...
describe('getBarClass', () => {
    ...
    it('returns b-evenly-allocated for parent when all children are even', () => {
        const leafA: TreeGroupResource = { id : 'A' };
        const leafB: TreeGroupResource = { id : 'B' };
        ...
        // Parent row
        const parentResource: TreeGroupResource = { id : 'P', children : [leafA, leafB] };
        const parentDom = makeDomConfig();
    ...
    it('returns b-mixed-state for parent when a child is under-allocated', () => {
        const leafGood: TreeGroupResource = { id : 'G' };
        const leafBad: TreeGroupResource = { id : 'B' };
        const tick = new Date('2026-03-01');
        ...
        const parentResource: TreeGroupResource = { id : 'P', children : [leafGood, leafBad] };
        const parentDom = makeDomConfig();
    ...
    it('returns b-mixed-state for parent with empty cache (first render)', () => {
        const leafA: TreeGroupResource = { id : 'A' };
        const parentResource: TreeGroupResource = { id : 'P', children : [leafA] };
        const parentDom = makeDomConfig();
    ...
    it('returns b-overallocated for over-allocated parent', () => {
        const parentResource: TreeGroupResource = { id : 'P', children : [] };
        const parentDom = makeDomConfig();
